...
model.add(Conv2D(10, (2,2), padding='same', input_shape=(32,32,3))) # 그리고 filters가 10개니까 1개였던 필터가 10개로 바뀜(28,28,10) padding='same'이므로 그대로
model.add(Dropout(0.2)) # 명시 해준만큼 뺀다
...
model.add(Dense(10, activation='relu')) #Dense층으로 왔기 때문에 ( ,100)으로 바뀜
...
model.fit(x_train, y_train, epochs=1000, batch_size=32, verbose=1, validation_split=0.2, callbacks=[early_Stopping]) #원래 배치 사이즈의 디폴트는 32
...
Dropout 적용 전
loss :  0.0568692572414875
acc :  0.9835000038146973
