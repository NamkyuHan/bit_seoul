...
for item in name:
    ...
    for i in range(len(find_imglist)):
        path = './project/imgtest/'+str(i)+'/'
        os.makedirs(path, exist_ok=True)    
        urlretrieve(find_imglist[i], path + str(i) + '.jpg')
        # print(find_imglist[i])

find_namelist = np.save('./project/imgtest/find_namelist.npy', arr=find_namelist)